...
def resolve_external_ref(ref, base_dir, source_file, location):
    if "#" not in ref:
        print(
            f"[WARNING] External ref missing anchor in {source_file} at {location}: {ref}"
        )
        return
    ...
    if not os.path.isfile(full_path):
        print(
            f"[ERROR] Missing file for external ref in {source_file} at {location}: {ref}"
        )
        return
    try:
        target_schema = load_yaml(full_path)
        resolve_internal_ref(
            "#/" + anchor.strip("/"), target_schema, full_path, location
        )
                    ...
                    for ref, location, source_file in refs:
                        if ref.startswith("#/"):
                            ...
                            resolve_external_ref(
                                ref, os.path.dirname(source_file), source_file, location
                            )
                        else:
                            print(
                                f"[WARNING] Unknown $ref format in {source_file} at {location}: {ref}"
                            )
